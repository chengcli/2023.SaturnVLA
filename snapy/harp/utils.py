...
def get_ray_out(inpfile, band = ""):
  inp = athinput(inpfile)

  # out directions
  if band != "":
    outdir = inp['radiation'][band + '.outdir'].split(' ')
  else:
...
def get_inversion_variables(inpfile, name):
  inp = athinput(inpfile)
  try:
    vlist = list(map(int, inp['inversion'][name + '.variables'].split()))
  except AttributeError:
    vlist = [int(inp['inversion'][name + '.variables'])]
  return vlist
